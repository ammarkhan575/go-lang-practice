...
func main() {
	...
	newFile, err := os.Create("23_files/newFile.txt")
	if err != nil {
	...
	// write into file by using byte
	bytes := []byte("Hello Ammar");
	newFile.Write(bytes)
	...
	destFile, err := os.Create("23_files/backup_files.txt")
	if err != nil {
		panic(err)
	}
	...
	// for reader we will use bufio (source file is reader interface)
	...
	for {
		...
		err_w := writer.WriteByte(b)
		if err_w != nil {

		}

	}
	// it is used to remove if there is any data remains in the writer then we need to flush it
	writer.Flush()
	...
	// err_d := os.Remove("23_files/backup_files.txt")
	// if err_d != nil {
	// 	panic(err_d)
	// }
